  ...
  //register on counterclockwise ring
    ...
    always@(*)begin
        if(reduction_ready) begin// the reduction unit has the highest priority when one reduction has collected all the expectations
            ...
            inject_consume_reduction=inject_consume;
        end
        else if(multicast_active) begin
            injector_in=reduction_unit_out;
            inject_consume_singlecast=0;
...
//this unit will collect the expect packet in each reduction entry, which will send a new packet when all the expectations are collected
	...
	assign ejector0_match=(ejector0_in[ExitPos+ExitWidth-1:ExitPos]==srcID);
	...
	assign ejector1_match=(ejector1_in[ExitPos+ExitWidth-1:ExitPos]==srcID);
	...
	//mux for ejector
	assign eject=(ejector0_out0[PriorityPos+PriorityWidth-1:PriorityPos]>ejector1_out0[PriorityPos+PriorityWidth-1:PriorityPos])?
					(ejector0_out0):(ejector1_out0);
	...
	always@(*)
		begin
		if(ejector0_in[ExitPos+ExitWidth-1:ExitPos]==srcID)//being ejected
			begin
			if(EjectSlotAvail)
				begin
				if(ejector0_out0[PriorityPos+PriorityWidth-1:PriorityPos]>ejector1_out0[PriorityPos+PriorityWidth-1:PriorityPos]&&(~fifo0_empty))
					begin
					eject_enable0<=1;
					fifo0_consume<=1;
				end
			...
			if(CounterClockwiseNextSlotAvail)
				begin
				if(injector_out0[PriorityPos+PriorityWidth-1:PriorityPos]<=ejector0_out1[PriorityPos+PriorityWidth-1:PriorityPos]&&(~fifo0_empty))
					begin
					fifo0_consume<=1;
				end
				else
	...
	always@(*)
		begin
		if(ejector1_in[ExitPos+ExitWidth-1:ExitPos]==srcID)//being ejected
			begin
			if(EjectSlotAvail)
				begin
				if(ejector0_out0[PriorityPos+PriorityWidth-1:PriorityPos]<=ejector1_out0[PriorityPos+PriorityWidth-1:PriorityPos]&&(~fifo1_empty))
					begin
					eject_enable1<=1;
					fifo1_consume<=1;
				end
			...
			if(ClockwiseNextSlotAvail)
				begin
				if(injector_out1[PriorityPos+PriorityWidth-1:PriorityPos]<=ejector1_out1[PriorityPos+PriorityWidth-1:PriorityPos]&&(~fifo1_empty))
					begin
					fifo1_consume<=1;
				end
				else
